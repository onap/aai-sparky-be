...
public class VnfSearchQueryBuilder {
	...
	/*
	 * { "vnfs" : { "text" : "VNFs", "completion" : { "field" :
	...
	public static void getSummaryAggsBlob(JsonObjectBuilder aggsBlobBuilder, String aggsKey, int resultSize) {
		JsonObjectBuilder fieldBuilder = Json.createObjectBuilder().add("field", aggsKey).add("size", resultSize);
	...
	public static void buildSingleTermCountQuery(JsonObjectBuilder jsonBuilder, String key, String value) {
		jsonBuilder.add("query", getTermBlob(key, value));
	}

	public static void buildSingleTermSummaryQuery(JsonObjectBuilder jsonBuilder, String key, String value,
			String groupByKey) {
		JsonObjectBuilder queryBlobBuilder = Json.createObjectBuilder();
		JsonObjectBuilder aggsBlobBuilder = Json.createObjectBuilder();

		queryBlobBuilder.add("constant_score", Json.createObjectBuilder().add("filter", getTermBlob(key, value)));
	...
	public static void buildMultiTermSummaryQuery(JsonObjectBuilder jsonBuilder, Map<String, String> attributes,
			String groupByKey) {
		JsonObjectBuilder queryBlobBuilder = Json.createObjectBuilder();
		JsonObjectBuilder aggsBlobBuilder = Json.createObjectBuilder();
	...
	public static void buildMultiTermCountQuery(JsonObjectBuilder jsonBuilder, Map<String, String> attributes) {
		JsonArrayBuilder mustBlobBuilder = Json.createArrayBuilder();
		for (String key : attributes.keySet()) {
			mustBlobBuilder.add(getTermBlob(key, attributes.get(key)));
		}
		jsonBuilder.add("query",
				Json.createObjectBuilder().add("bool", Json.createObjectBuilder().add("must", mustBlobBuilder)));
	}

	public static JsonObject createSummaryByEntityTypeQuery(Map<String, String> attributes, String groupByKey) {
		JsonObjectBuilder jsonBuilder = Json.createObjectBuilder();
	...
	public static JsonArray getSortCriteria(String sortFieldName, String sortOrder) {
		...
		jsonBuilder
				.add(Json.createObjectBuilder().add(sortFieldName, Json.createObjectBuilder().add("order", sortOrder)));
