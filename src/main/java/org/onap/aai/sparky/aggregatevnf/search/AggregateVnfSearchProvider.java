...
public class AggregateVnfSearchProvider implements SearchProvider {
	...
	private static final String AUTO_SUGGEST_TEMPLATE = "{ " + "\"results-size\": %d," + "\"suggest-text\": \"%s\","
			+ "\"suggest-field\": \"%s\"" + "}";
	...
	public AggregateVnfSearchProvider(SearchServiceAdapter searchServiceAdapter, String autoSuggestIndexName,
			String vnfSearchSuggestionRoute) {
				...
				while (nodeIterator.hasNext()) {
					...
					if (sourceNode.has(KEY_FILTER_LIST)) {
						...
						for (int i = 0; i < filtersArray.size(); i++) {
							...
							UiFilterValueEntity filterValue = mapper.readValue(filterValueString,
									UiFilterValueEntity.class);
